...
/**
 * Creates one server-authoritative Free Lobby Event every 2 minutes by delegating
 * to the locked, window-keyed {@link BattleLobbyService#createEventLobby()}.
 *
 * <p>The cron fires on each 2-minute boundary; the event lobby's {@code event_window_key}
 * is derived from the wall clock (floored to the 2-minute window), so a late run
...
public class FreeLobbyEventScheduler {
    ...
    @Scheduled(cron = "${valocase.lobby.event-cron:0 */2 * * * *}", zone = "UTC")
    public void run() {
        try {
